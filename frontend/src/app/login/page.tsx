...
export default function Login() {
  ...
  const saveUser = async (userCredential: UserCredential) => {
    try {
      const uid = userCredential.user.uid;
      let name = userCredential.user.displayName;
      let email = userCredential.user.email;
      const user = { name, email, uid };
      await axios.post(`${process.env.NEXT_PUBLIC_API_HOST}/api/user`, user);
    } catch (error) {
      console.log(error);
    }
  };
              ...
              <div className="divide-y divide-gray-200">
                ...
                >
                  ...
                  <span className="text-sm">
                    Forgot Password?{" "}
                    <Link className="text-blue-700" href="/reset-password">
                      reset!
                    </Link>
                  </span>
