...
export default function EditBlog() {
  ...
  //   setArticle(allBlogs.find((itme) => slugify(itme.title) === slug));
  //   setTitle(article?.title || "")
  //   setImages([article?.img] || "")
  //   setDisplayImages([article?.img])
  //   setContent(article?.content || "")
  // }, [article?.content, article?.img, article?.title, slug]);

  useEffect(() => {
    async function getData() {
      await fetch(`${apiHost}/api/blogs/article`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ slug: decodeURIComponent(slug as string) }),
      })
        .then(async (res) => {
  ...
  const handleSave = async () => {
    ...
    formData.append("slug", decodeURIComponent(slug as string));
    formData.append("content", content);
    if (images.length > 0) {
      formData.append("image", images[0]);
    }
    // const data = {title, slug: slug, content}
    setLoading(true);
    try {
        ...
        <div className="mb-28 mt-3 max-w-lg mx-auto">
          ...
          <div className="mt-10">
            ...
            >
              {loading ? (
                <LoaderSpinner
                  loaderStyle="size-5 stroke-white"
                />
              ) : (
